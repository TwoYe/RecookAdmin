...
class A extends React.Component {
    ...
    t = () => {
        ...
        else if (this.state.fj == null || this.state.fj == "") this.setState({ Vail_name: { vail: "", help: "" }, Vail_fj: { vail: "error", help: "父级类目项必选" } })
        else if (A.length == 0) this.setState({ Vail_fj: { vail: "", help: "" }, Vail_lmt: { vail: "error", help: "请上传类目图" } })
        else {
            this.setState({ Vail_lmt: { vail: "", help: "" }, loading: true });
            baseApi(`/api/v1/admin/goods/categories/entry`, {
                name: this.state.name,
                parentID: this.state.fj[0],
                depth: 2,
                logoUrl: A[0].name
            }, (res) => {
                if (res.code == "0_0") {
            ...
            <div>
                ...
                <Form {...formItemLayout} style={{ width: "40%", margin: "0 auto" }}>
                    ...
                    <Form.Item
                        label="父级类目"
                        required
                        validateStatus={this.state.Vail_fj.vail}
                        help={this.state.Vail_fj.help}
                    >
                        <Cascader
                            placeholder="请选择"
                            onChange={value => this.setState({ fj: value })}

...
class A extends React.Component {
    ...
    save = () => {
        if (this.props.name == "") message.error("模板名称不可以为空！")
            ...
            A.push({
                firstWeight: mr_sj,
                firstWeightFee: mr_sf,
                additionalWeight: mr_xj,
                additionalWeightFee: mr_xf,
                children: mr
            });

            for (let i = 0; i < sj.length; i++) {
                if (area[i] != undefined && sj[i] != undefined && sf[i] != undefined && xj[i] != undefined && xf[i] != undefined) {
                    A.push({
                        firstWeight: sj[i].value == null ? 1 : sj[i].value,
                        firstWeightFee: sf[i].value == null ? 1.0 : sf[i].value,
                        additionalWeight: xj[i].value == null ? 1 : xj[i].value,
                        additionalWeightFee: xf[i].value == null ? 1.0 : xf[i].value,
            ...
            baseApi(`/api/v1/admin/freight_template/weight/add`, B,
                (res) => {
                    if (res.code == "0_0") {
                        Modal.success({
                            title: '模板添加成功！',
    ...
    sc = (e, index, item) => {
        ...
        this.setState({ data: A, area: area, lock: BJ, sj: sj, sf: sf, xj: xj, xf: xf })
                    ...
                    </tr>
                    {
                        (this.state.data).map((item, index) =>
                            ...
                            <tr key={item} style={{ height: "40px" }}>
                                ...
                                <td>
                                    <InputNumber min={1} onChange={value => this.updateSJ(value, item, "sj")} step={0.1}/>
                                </td>
                                <td>
                                    <InputNumber min={1} onChange={value => this.updateSJ(value, item, "sf")} step={0.1} />
                                </td>
                                <td>
                                    <InputNumber min={1} onChange={value => this.updateSJ(value, item, "xj")} step={0.1}/>
                                </td>
                                <td>
                ...
                <Alert
                    message={
                        <span>
                            ...
                            <InputNumber min={1} defaultValue={1} onChange={value => this.setState({ mr_sj: value })} step={0.1}/>
                            <span> Kg 内 </span>
                            <InputNumber min={1} defaultValue={1.0} onChange={value => this.setState({ mr_sf: value })} step={0.1} />
                            <span> 元，每增加 </span>
                            <InputNumber min={1} defaultValue={1} onChange={value => this.setState({ mr_xj: value })} step={0.1}/>
                            <span> Kg ，增加运费 </span>
